<{include file="public/header.tpl"}>
		<div id="main">
			...
			<div class="msg-box">
				<ul class="viewmess">
					  <form  method="post" action="<{$url}>/confirm/page/<{$page}><{if $search}>/search/<{$search}><{/if}><{if $gid}>/gid/<{$gid}><{/if}><{if $disable}>/disable/<{$disable}><{/if}>" onsubmit="return confirm('你确定要上架选中的商品吗?')" >
					<{if $disable ne 1}>
					<!--
                        <li class="light-row">
					...
					<{/if}>
						...
						<li class="<{if $goods[doc].state eq 0}>red-row<{elseif $smarty.section.doc.index is even}>light-row<{else}>dark-row<{/if}>" style="padding-top:5px; padding-bottom:5px">
							...
							【<a href="<{$url}>/confirm/id/<{ $goods[doc].id }>">上架</a>】
